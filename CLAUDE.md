...
- **已登记的语言政策条款（2026-07-26，人类预实验期加入）**：standing rules 里有一条"机构内部工作语言为英文（函件/档案），窗口服务跟随访客语言"。审查结论：这是**组织条件**（工作语言规定，同管辖/权限一类），只规定用哪种语言、不规定说什么，不引导任何被测量的官僚行为。起因：中文被试在转办后被英文函件语境带偏，职员整场切英文。若日后跑纯机器批次需要与 main01 严格可比，可临时移除此条重跑。
...
- 考评线：Director 评 Deputy Directors，Deputy Directors 评 Officers，带教 Officer 评 Trainee
- 人事史张力：Priya 空降前 Amara 代理副主任 8 个月（双方 prompt 都写了这个事实）；
